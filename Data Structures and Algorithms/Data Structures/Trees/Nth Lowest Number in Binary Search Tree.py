# We are given a Binary Search Tree(BST) and a node number nn. We have to find the node with the nth lowest value.
...
class Solution:
    ...
    def kthLowest(self, root: Optional[BinaryTreeNode], k: int) -> int:
        # since BSTs in-order traversal gives us a sorted list
        # perform in-order traversal and count the number of seen nodes
        # kth node seen will be the lowest
        if not root:
            return None
        res = self.kthLowest(root.left, k)
        if res != None:
            return res
        self.seen_nodes += 1
        if self.seen_nodes == k:
            return root
        res = self.kthLowest(root.right, k)
        if res != None:
            return res
        return None
...
n = 2
current_count = 0
nth_highest_node = sol.kthLowest(root, n)
